...
<body style="margin: 0; padding: 0; background-color: ${this.bgColor}; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
    ...
    renderWelcome(name, email, tempPassword) {
        const content = `
    ...
    renderDiagnosticInvitation(name, magicLink) {
        const content = `
            <h2 style="margin: 0 0 20px; color: ${this.textColor}; font-size: 22px;">
                Bonjour ${name},
            </h2>
    ...
    renderAppointmentConfirmation(name, date, type, meetingLink, agencyAddress) {
    ...
    renderPaymentConfirmation(name, amount, refundCode) {
        const content = `
            <h2 style="margin: 0 0 20px; color: ${this.textColor}; font-size: 22px;">
                Paiement reçu ! 💳
            </h2>
    ...
    renderAppointmentReminder(name, date, type, meetingLink) {
        const dateStr = date.toLocaleDateString('fr-FR', {
            weekday: 'long',
            day: 'numeric',
            month: 'long',
